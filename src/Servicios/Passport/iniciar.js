...
//                 if (!usuario || !_id) return console.log("usuario disponible para registrar");
//                 if (error) return console.log("Error al buscar un usuario ya registrado");
//             })
...
const iniciar = () => {
    ...
    passport.use("signup", new LocalStrategy({
        ...
        try {
            if (!usuario || !contraseña) return respuesta.send({ success: false })
            console.log(`usuario: ${usuario}, contraceña: ${contraseña}`)
            ...
            const crearUsuario = await DaoUsuario.guardar(nuevoUsuario, (error, nuevoUsuario) => {
                ...
                console.log(`Usuario ${UsuarioNuevo.email} registrado correctamente`);
